...
interface DailyVisitorChartProps {
...
export const DailyVisitorChart = ({ defaultPeriod = '7d' }: DailyVisitorChartProps) => {
  ...
  const fetchVisitorData = async (period: string) => {
    try {
      setLoading(true);
      setError(null);

      const response = await fetch(`/api/cs2dle/analytics/visitors/daily?period=${period}`);
      ...
      <Card className="h-full flex flex-col">
        ...
        <CardContent className="flex-1">
          <div className="text-center py-8">
            ...
            <Button onClick={() => fetchVisitorData(selectedPeriod)}>Try Again</Button>
          </div>
        </CardContent>
      </Card>
    );
  ...
  const chartData = data.dailyVisitors.map(item => ({
    date: format(parseISO(item.date), 'MMM dd'),
    fullDate: format(parseISO(item.date), 'MMM dd, yyyy'),
    uniqueVisitors: item.uniqueVisitors,
    totalVisits: item.totalVisitors,
  }));
  ...
  if (data.dailyVisitors.length === 0) {
    return (
      <Card className="h-full flex flex-col">
        <CardHeader className="flex-shrink-0">
          <CardTitle>Daily Visitors</CardTitle>
          <CardDescription>Track visitor trends over time</CardDescription>
        </CardHeader>
        <CardContent className="flex-1">
      ...
      <CardHeader className="flex-shrink-0">
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950 dark:text-purple-300 dark:border-purple-800">
            ...
            Unique Visitors: {data.summary.totalVisitors.toLocaleString()}
          </Badge>
          ...
          <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800">
            ...
            Avg/Day: {data.summary.averageVisitorsPerDay.toFixed(1)}
          </Badge>
          {data.summary.peakDay && (
            <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-950 dark:text-orange-300 dark:border-orange-800">
              <Calendar className="h-3 w-3 mr-1" />
              Peak: {format(parseISO(data.summary.peakDay.date), 'MMM dd')} ({data.summary.peakDay.visitors} visitors)
            </Badge>
          )}
        </div>
      </CardHeader>

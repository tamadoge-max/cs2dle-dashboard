...
const InactiveUsersExportButton = ({ daysThreshold }: InactiveUsersExportButtonProps) => {
  ...
  const handleExport = async (format: 'json' | 'csv' | 'excel') => {
    ...
    try {
      ...
      setCurrentStep('fetching');
      const response = await fetch(`/api/cs2dle/users/inactive-export?days=${daysThreshold}`);
      ...
      switch (format) {
        case 'json':
          ...
          filename = `inactive-users-${daysThreshold}-days.json`;
          break;
        case 'csv':
          blob = createCSVBlob(data);
          filename = `inactive-users-${daysThreshold}-days.csv`;
          break;
        case 'excel':
          blob = createExcelBlob(data);
          filename = `inactive-users-${daysThreshold}-days.xlsx`;
          break;
        default:
  ...
  const getStepText = (step: ExportStep) => {
    switch (step) {
      case 'fetching':
        return 'Fetching inactive users from database...';
      case 'converting':
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 w-96 max-w-[90vw] mx-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Exporting Inactive Users</h3>
                  ...
                  return (
                    <div key={step} className="flex items-center gap-3">
                      ...
                      <span className={`text-sm ${
                        ...
                        {isCompleted ? 
                          step === 'fetching' ? 'Inactive users fetched successfully' :
...
export default InactiveUsersExportButton;